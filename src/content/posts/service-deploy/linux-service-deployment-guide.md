---
title: "Linux 环境常用服务部署指南"
published: 2026-08-04
description: "本文档整理了在 Linux 环境下手动编译安装与配置 Nginx、MySQL、Tomcat、Docker、Redis 及 OpenGauss 的详细步骤与系统服务配置。"
category: "服务部署"
tags:
  - "Nginx"
  - "MySQL"
...
可选：配置 www 用户管理，需要 www 用户和用户组，可参考下面 MySQL 添加。
...
**注意：MySQL 8.0 开始才支持 arm 架构，我们可以去第三方下载编译好的安装包，或者可以采取 Docker 安装。**
...
下载 MySQL 文件：[下载链接](https://obs.cn-north-4.myhuaweicloud.com/obs-mirror-ftp4/database/mysql-5.7.27-aarch64.tar.gz)
...
# 启用 binlog 日志文件，可以指定目录，如果不指定则放在数据目录下面
log_bin = mysql-bin
...
# 临时数据路径
tmpdir = /usr/local/mysql/mysql_tmp
...
```bash
docker run -d --name redis -p 6379:6379 redis:6.2.3
```
...
数据库安装完成后，默认生成名称为 postgres 的数据库。也可以连接默认的 postgres 数据库来验证：
