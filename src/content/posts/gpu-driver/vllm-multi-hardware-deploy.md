---
title: "多硬件平台 vLLM 部署与推理指南"
published: 2026-08-04
description: "本文档详细记录了在 Nvidia、AMD、华为昇腾及纯 CPU 环境下部署 vLLM 服务、创建容器及验证推理的完整流程与参数说明。"
category: "GPU与驱动"
tags:
  - "vLLM"
  - "Docker"
...
`enforce-eager` 
是否强制开启 Pytorch 的 eager 模式，默认关闭，此时会额外使用 CUDA graph 做进一步加速，但会占用额外显存，并增加一些服务启动耗时。若启动报显存不足，可以尝试启动命令中添加 `--enforce-eager` 参数来节约显存占用，但是推理性能会稍有下降

`served-model-name`
启动的服务名称

`gpu-memory-utilization`
GPU 使用显存比例，值越大占用显存越高
...
每次迭代最大处理的 token 数，默认取 2048 和模型支持的上下文长度的较大值；若显存富余较多，且输入文本较长，可以适当调大此参数，以获得更好的吞吐性能。

`api-key` "1!Deshine"
兼容 OPEN-API 的密钥，有特殊符号需要加 `""`
...
**注意：MI210 在 Ubuntu 22.04 下需要在 `/etc/default/grub` 下添加 `GRUB_CMDLINE_LINUX_DEFAULT="amd_iommu=off"`，否则只能使用单卡。**
...
注：2.0 以上版本的镜像可能需要设置依赖库的环境变量，MINDIE 的 docker 镜像自带了环境变量一键脚本
```bash
...
# 设置 KV 缓存大小为 40G，更多的缓存可以允许更多的并发。

`VLLM_CPU_OMP_THREADS_BIND=21-25`
# 指定 CPU 内核为 21-25 核
